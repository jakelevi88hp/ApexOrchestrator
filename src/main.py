...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={"ok": False, "error": "Internal server error"}
    )
...
async def add_security_headers(request: Request, call_next):
    ...
    if request.url.path in ["/docs", "/redoc", "/openapi.json"]:
        ...
        response.headers["Content-Security-Policy"] = (
            "default-src 'self'; script-src 'self' 'unsafe-inline' cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' cdn.jsdelivr.net; img-src 'self' data: cdn.jsdelivr.net"
        )
    else:
...
def run_python(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        proc = subprocess.run(
            f'python "{pyfile}"', shell=True, capture_output=True, text=True, timeout=timeout, cwd=str(WORK_DIR)
        )
...
async def http_request(method: str, url: str, headers: Dict[str, str] = None, body: Any = None) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout) as c:
            ...
            result = {"status": r.status_code, "headers": dict(r.headers), "text": r.text[:20000]}
...
async def run_step(step: ToolCall, run_id: str) -> Dict[str, Any]:
    ...
    if step.tool == "file_write":
        res = file_write(
            step.args.get("path", "artifact.txt"), step.args.get("content", ""), bool(step.args.get("overwrite", True))
        )
        ...
        res = await http_request(
            step.args.get("method", "GET"),
            step.args.get("url", ""),
            step.args.get("headers") or {},
            step.args.get("body"),
        )
...
async def metrics(request: Request):
    ...
    return {
        ...
        "logs": {"count": len(log_files), "total_size_mb": sum(f.stat().st_size for f in log_files) / (1024 * 1024)},
        "work_dir": {"path": str(WORK_DIR), "exists": WORK_DIR.exists()},
    }
