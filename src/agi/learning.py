...
class AcceleratedLearner:
    ...
    def __init__(self, memory_system):
        ...
        self.techniques = {
            ...
            "reinforcement_learning": self._apply_active_learning,  # Use active learning as fallback
        }
    ...
    async def learn_from_interaction(
        self, input_data: Any, perception_result: Dict, reasoning_result: Dict, action_plan: Dict
    ):
        """
    ...
    async def _extract_session_patterns(self, session: Dict) -> List[Dict]:
        ...
        if reasoning_result.get("insights"):
            for insight in reasoning_result["insights"]:
                patterns.append({"type": "reasoning", "description": insight, "confidence": 0.7})

        # Extract action patterns
        action_plan = session.get("action_plan", {})
        if action_plan.get("steps"):
            for step in action_plan["steps"]:
                patterns.append({"type": "action", "description": step.get("description", ""), "confidence": 0.8})
