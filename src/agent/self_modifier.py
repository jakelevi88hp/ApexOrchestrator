...
class SelfModifier:
    ...
    async def propose_modification(
        self, target_file: str, modification_type: str, description: str, reason: str
    ) -> Dict[str, Any]:
        ...
        if modification_type == "optimize":
            ...
            return {"status": "error", "error": f"Unknown modification type: {modification_type}"}
    ...
    async def apply_modification(self, proposal_id: int, auto_test: bool = True) -> Dict[str, Any]:
        ...
        if current_hash != proposal["current_code_hash"]:
            return {"status": "error", "error": "File has been modified since proposal was created"}
        ...
        if auto_test:
            ...
            if test_results["status"] == "failed":
                ...
                return {"status": "test_failed", "test_results": test_results}
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "rollback": "attempted"}
    ...
    def rollback_modification(self, modification_id: int) -> Dict[str, Any]:
        ...
        return {"status": "rolledback", "message": "Modification reverted to backup"}
    ...
    async def _add_feature(self, code: str, description: str) -> Dict[str, Any]:
        ...
        feature_code = await self.code_generator.generate_function(purpose=description, requirements={})

        # Merge with existing code
        merged = code + "\n\n" + feature_code.get("code", "")

        return {"code": merged, "description": f"Added feature: {description}"}
    ...
    async def _fix_bug(self, code: str, description: str) -> Dict[str, Any]:
        ...
        return {"code": fixed, "description": f"Fixed bug: {description}"}
    ...
    async def _refactor_code(self, code: str, reason: str) -> Dict[str, Any]:
        ...
        return {"code": refactored, "description": f"Refactored: {reason}"}
    ...
    def _git_commit(self, file_path: str, message: str):
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", f"[AutoAgent] {message}"], check=True)
